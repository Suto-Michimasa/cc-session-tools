...
`~/.claude/daily-report-config.json` を作成して動作をカスタマイズできます:
...
| `templatePath` | `~/.claude/daily-report-template.md` | カスタムテンプレートのパス |
| `language` | `en` | テンプレート言語 — カスタム templatePath 未設定時に `templates/{language}.md` を選択 |
...
```
cc-daily-report/
└── daily-report/              # スキル本体（~/.claude/skills/ にコピーまたはリンク）
